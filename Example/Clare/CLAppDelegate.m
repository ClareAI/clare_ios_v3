...
//  Clare
//
//  Created by workpanda on 03/06/2020.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    settings.host = @"Your Host";
    
   //,@"zh_TW", @"ja_JP", @"vi_VN",@"tl_PH"
    
   // settings.languages = [[NSArray alloc]initWithObjects:@"zh_HK",@"zh_CN",@"en_US",@"zh_TW", @"ja_JP", @"vi_VN",@"tl_PH", nil];//5
    ...
    settings.avatarImage = [UIImage imageNamed:@"insurance" inBundle:[NSBundle bundleForClass:[self class]] compatibleWithTraitCollection: nil]; //3
    
    settings.disclaimers = [[NSMutableDictionary alloc]init];
    [settings.disclaimers setObject:@[@"<span>• Virtual Assistant is an automated service, and therefore cannot answer questions specific to your account.<br>• Please do NOT disclose any personal account information in your messages.<br></span>"] forKey:@"en_US"];//4   //,@"<span>• The information you input here will be recorded for service quality assurance.<br>• Data will be processed and stored by members of the Fidelity Group and authorised third parties, which may include processing overseas.<br>• For the details please read the <a href=\"hktv://www.hktvmall.com/p/H0883004_S_10214181\">Security and Privacy</a> guidelines.</span>"
    ...
    settings.languages = [[NSArray alloc]initWithObjects: @"en_US",nil]; //5
    settings.microphoneEnable = YES; //6
    settings.voiceEnable = NO; //7
    settings.isReplyButtonInOneRow = YES; //8
    ...
    styleSetting.quickReplyButtonBgColor = UIColor.cyanColor;
    styleSetting.quickReplyButtonFontColor = UIColor.whiteColor;
    styleSetting.quickReplyButtonBorder = YES;
    ...
    styleSetting.headerBarBgColor = UIColor.blueColor;
    styleSetting.userBubbleBgColor = UIColor.whiteColor;
    styleSetting.userBubbleBorder = YES;
    styleSetting.userBubbleFontColor = UIColor.blueColor;
    styleSetting.postbackBgColor = UIColor.blackColor;
    styleSetting.postbackBorder = YES;
    styleSetting.postbackFontColor = UIColor.whiteColor;
    styleSetting.searchResultFontColor = UIColor.blueColor;

    [[Clare sharedManager] setStyle:styleSetting];
     
    NSMutableDictionary *properties = [[NSMutableDictionary alloc]init]; //11
    [properties setObject:@"your triggerFlowID" forKey:@"triggerFlowId"];
