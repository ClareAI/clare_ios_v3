...
//  Clare
//
//  Created by vernalclareai on 02/01/2021.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [settings.disclaimers setObject:@[@"<span>• Virtual Assistant is an automated service, and therefore cannot answer questions specific to your account.<br>• Please do NOT disclose any personal account information in your messages.</span>"] forKey:@"en_US"];//4
    //,@"<span>MallMall Chat is an automated service, and information provided for general enquiries is for reference only. Please do not disclose any personal information during the conversation.</span>"
    ...
    settings.microphoneEnable = NO; //6
    settings.voiceEnable = NO; //7
    settings.isReplyButtonInOneRow = NO; //8
    settings.loadHistory = NO; //9
    settings.languageDetection = YES; //10
    settings.sendGreetingAgain = YES; // 11
    settings.autoStart = NO; // 12 if No, it will not make the connection to the chat server until the chatwidget is opened.
